        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $staparams['statement_inoperator'] = $this->auth->nickname;//出操作员即可当前操作
                ...
                if ($result !== false) {
                	if($m!=='') {
                	  //发送短信
             	     $sendrul = 'http://api.smsbao.com/sms?u=ha_hlgp&p=6ab37e4a4b3b3f7a06a0227acffd4240&m='.$m.'&c=【汇隆果品】尊敬的'.urlencode($n).'，您本次缴费'.urlencode($a).'元，账户余额为'.urldecode($b).'元。';//.urlencode($content);
